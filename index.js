...
// routes
app.get("/", (req, res) => {
  res.send("<h1>asasdfa</h1>");
});

// create a river
app.post("/river", async (req, res) => {
  console.log("bing");
  console.log(req.body);
  try {
    console.log(req.body);
    const { state, watershed, river, geoTag, usgsId } = req.body;
    const newRiver = await pool.query(
      "INSERT INTO river (state, watershed, river, geoTag, usgsId) VALUES($1 RETURNING *",
      [state, watershed, river, geoTag, usgsId]
    );
  } catch (err) {
